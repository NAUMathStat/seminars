...
**Abstract:** 
Dynamical systems are Ordinary Differential Equations (ODEs) of the form $\frac{dx}{dt} = f(x)$, or iterated maps of the form $x_{i+1} = f(x_i)$, where $f: \mathbb{R}^n \to \mathbb{R}^n$. A symmetry arises when $f(Px) = P f(x)$ for some matrix $P$.

An ODE with parameters $\mu \in \mathbb{R}^p$ has the form $\frac{dx}{dt} = f(x, \mu)$, and $f$ has a parameter symmetry if $f(Px, Q\mu) = P f(x,\mu)$ for a pair of matrices $(P, Q)$. A parameter symmetry relates the dynamical system with parameters $\mu$ to the dynamical system with parameters $Q\mu$.
